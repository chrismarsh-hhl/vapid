...
import ChoiceDirectiveFunction from '../../lib/directives/choice';

const ChoiceDirective = ChoiceDirectiveFunction(BaseDirective);
...
describe('.constructor', () => {
  test('parses options into an array', () => {
    ...
    expect(directive.possibilities).toEqual(
      expect.arrayContaining(['Yes', 'No', 'Maybe']),
    );
  });
...
describe('#input', () => {
  ...
  test('renders a radio if there are <= 3 options or if input=radio', () => {
    ...
    const alsoGood = new ChoiceDirective({
      input: 'radio',
  ...
  test('allows multi-select dropdown', () => {
    const multiple = new ChoiceDirective({
      options: 'Yes, No, Maybe, Other',
      multiple: true,
    });
    ...
    const notMultiple = new ChoiceDirective({
      options: 'Yes, No, Maybe, Other',
    });
