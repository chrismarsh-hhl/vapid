...
import { notFound, boomify } from '@hapi/boom';

import { UriPathAnalyzer } from '../services/index.js';
import TemplateCompiler from '../TemplateCompiler/index.js';
import { Logger, Paths, Utils } from '../utils/index.js';
 ...
 */
export async function renderContent(uriPath, extension) {
  const content = {};
  const { Section } = this.db.models;
  const pathAnalyzer = new UriPathAnalyzer(uriPath, extension, this.paths.www);
  const [file, pathSection, pathRecordId] = pathAnalyzer.perform();

  if (!file) {
    throw notFound(`Template not found: ${uriPath}${extension}`);
  }

  const partials = glob.sync(resolve(this.paths.www, '**/_*.html'));
  const template = TemplateCompiler.fromFile(file, this.paths.www, partials);
  ...
  for (const [sectionToken, section] of entries) {
    const recordId =
      pathSection === section.name && section.params.multiple !== 'true'
        ? pathRecordId
        : null;
 ...
 */
export function renderError(err, request) {
  ...
  if (this.isDev && status !== 404) {
    ...
    errorFile =
      status === 404 && fs.existsSync(siteFile)
        ? siteFile
        : resolve(viewsPath, 'errors', `${status}.html`);
...
function _addPlaceholders(content, section, db) {
  ...
  if (content.length === 0) {
    const placeholders = Object.entries(section.fields || {}).reduce(
      (memo, [token, params]) => {
