...
class TemplateCompiler {
   ...
   */
  render(content = {}) {
    const body = !Utils.isEmpty(content.general)
      ? _wrapHTML(this.html)
      : this.html;
...
function _addToTree(tree, branchToken, leafToken) {
  /* eslint-disable no-param-reassign */
  const leafValue = Utils.merge(
    tree[branchToken].fields[leafToken] || {},
    parseLeaf(leafToken),
  );
