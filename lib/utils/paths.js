 ...
 */
export const getProjectPaths = function getProjectPaths(cwd, dataPath) {
  const paths = {
 ...
 * @return {Object} absolute paths
 */
export const getDashboardPaths = function getDashboardPaths() {
  const paths = {
 ...
 */
export const isTemplatePartial = function isTemplatePartial(filePath) {
 ...
 */
export const isAssetPath = function isAssetPath(filePath) {
  const ext = path.extname(filePath);
  ...
  if (
    ...
    XML_FILE_EXTS[ext] ||
    JSON_FILE_EXTS[ext] ||
    filePath.match(/.pack\.[js|scss|sass]/)
  ) {
    return false;
  }
 ...
 * @param {string} path
 */
export const assertPublicPath = function assertPublicPath(filePath) {
  const fileName = path.basename(filePath);
  const char = fileName.slice(0, 1);

  if (char === '_' || char === '.') {
    throw Boom.notFound(
      'Filenames starting with an underscore or period are private, and cannot be served.',
    );
  }
};
