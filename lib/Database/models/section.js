...
const sectionFunction = (sequelize, DataTypes) => {
  /*
      ...
      getterMethods: {
        ...
        tableColumnsHeaders: function tableColumnsHeaders() {
          return this.tableColumns.map(
            (key) => this.fields[key].label || Utils.startCase(key),
          );
        },
          ...
          return Object.entries(this.fields)
            ...
            .sort(
              (a, b) =>
                ...
                isNaN(a.priority) - isNaN(b.priority),
            );
        },
      },
  ...
  Section.rebuild = async function rebuild(name, params) {
    ...
    const fields = this.sequelize.models.Record.removeSpecialFields(
      params.fields,
    );
  ...
  Section.contentFor = async function contentFor(args, recordId) {
    ...
    if (args.keyword === 'form') {
      ...
      const formTemplate = fs.readFileSync(
        resolve(__dirname, '../../views/records/_form_email.ejs'),
        'utf8',
      );
    ...
    return Promise.all(
      records.map(async (record) => record.contentFor(args.fields)),
    );
  };
   ...
   */
  Section.destroyExceptExisting = function destroyExceptExisting(
    existing = [],
  ) {
    this.destroy({
      where: {
