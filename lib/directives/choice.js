...
const choiceFunction = (BaseDirective) => {
  /**
  ...
  class ChoiceDirective extends BaseDirective {
    ...
    constructor(params) {
      ...
      this.attrs.required =
        this.possibilities.length > 1 && this.attrs.required;
    }
    ...
    input(name, value = this.options.default) {
      ...
      return this.possibilities.reduce(
        (memo, p) => memo + this._checkbox(name, value, p, p),
        '',
      );
    }
    ...
    _checkbox(name, value, inputValue, label = '') {
      ...
      const type =
        this.options.input === 'toggle' ? 'checkbox' : this.options.input;
      const checked =
