...
import { Logger, Utils } from '../../utils/index.js';
...
class VapidDeployer extends Vapid {
  ...
  async deploy() {
    ...
    const presignedPosts = await this.getPresignedPosts(
      siteAndUploadsChecksums,
    );
  ...
  async ensureLogin() {
    ...
    Logger.info(
      'Please enter your Vapid credentials, or visit vapid.com to signup.',
    );
    ...
    const response = await ky.post(endpoint, { body, json: true });
  ...
  async ensureSite() {
    ...
    writePackage.sync(this.paths.pjson, pjson);
    ...
    if (deployContent) {
      ...
      if (fs.existsSync(this.paths.uploads)) {
        Utils.copyFiles(
          this.paths.uploads,
          path.resolve(this.tmpDir, 'uploads'),
        );
      }
    }
  ...
  getChecksums() {
    ...
    glob
      .sync(path.resolve(this.tmpDir, '**/!(*.pack.+(s[ac]ss|js))'), {
        mark: true,
      })
      .forEach((file) => {
        if (file.endsWith('/')) {
  ...
  async getContent() {
    ...
    const order = Section.DEFAULT_ORDER.map((o) => {
      o.unshift('records');
      return o;
    });
    const contentSections = await Section.scope('content').findAll({
      include: 'records',
      order,
    });
    return contentSections.reduce((memo, section) => {
      /* eslint-disable-next-line no-param-reassign */
      memo[section.name] = section.records.map((record) =>
        record.get('content'),
      );
      return memo;
    }, {});
  ...
  async getPresignedPosts(checksums) {
    const endpoint = url.resolve(
      this.apiURL,
      `/sites/${this.siteId}/presigned_posts`,
    );
  ...
  async uploadFiles(presignedPosts) {
    ...
    Object.keys(presignedPosts).forEach(async (pth) => {
      ...
      if (!post) {
        ...
        const promise = limit(() =>
          ky.post(post.url, { body: form }).then(() => {
