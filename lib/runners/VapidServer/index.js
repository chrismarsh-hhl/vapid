...
class VapidServer extends Vapid {
  ...
  async initialize(cwd) {
    ...
    const wpMiddleware = await webpackMiddleware(this.isDev, [this.paths.www], this.paths.modules);

    // Middleware
    app
      .use(securityMiddleware())
      .use(sessionMiddleware(app))
      // .use(webpackMiddleware(this.isDev, [dashboard.paths.assets, this.paths.www], this.paths.modules)) // eslint-disable-line max-len
      .use(wpMiddleware) // eslint-disable-line max-len
