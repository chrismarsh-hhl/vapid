...
import { renderContent } from '../../Renderer/index.js';
...
class VapidBuilder extends Vapid {
  /**
   * Initializes the builder, uses await to get package.json data
   */
  ...
  async build(dest) {
    if (!path.isAbsolute(dest)) {
      throw new Error(
        'Vapid build must be called with an absolute destination path.',
      );
    }
    ...
    const faviconPath = Utils.findFirst('favicon.ico', [
      this.paths.www,
      Paths.getDashboardPaths().assets,
    ]);
    if (faviconPath) {
    ...
    const templates = await glob(
      path.join(this.paths.www, '**/*.{html,xml,rss,json}'),
    );
    ...
    for (const tmpl of templates) {
      ...
      if (Paths.isTemplatePartial(tmpl)) {
        ...
        if (!section || !section.multiple) {
          continue;
        }

        const records = await section.getRecords();
        for (const record of records) {
          await this.renderUrl(
            record.permalink,
            path.join(dest, `${record.permalink}.html`),
          );
        ...
        const name =
          parsed.name === 'index' && parsed.dir ? parsed.dir : parsed.name;
